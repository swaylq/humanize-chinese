...
**用了 skill，四段跑完** 🟢 23/100 LOW（降 **63 分**）：
...
四段 = ① 按写作指南生成 → ② 按清单复查改写 → ③ `./humanize replace` 词表收尾 → ④ 纠错（只修错词、病句、标点，其他不动）。
...
>
> 有一句让我在换乘通道里站住了。哲人说，你不是因为愤怒才大声，是为了大声才制造出愤怒。前一晚我刚跟人吵过，当时觉得自己是被气到了，那会儿站在人流里突然觉得不太对。倒回去听了三遍。
>
...
| ④ 纠错收尾 | LLM（按 skill 纠错清单） | 只修错词、病句、标点，其他一个字不动 —— 改动范围有硬校验，越权整体作废 |
