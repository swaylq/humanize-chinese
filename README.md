...
实测（1-5 分，三模型评审；结果存疑时加跑一轮取并集）：

| | 通顺度 | 评审发现的病句 |
|---|---|---|
| 本工具 · 仓库五份样例 | 4.0 - 5.0 | 未发现 |
| 上一版纯规则改写 · 同样本 | 2.0 | 5 - 17 |
| 本工具 · 当前模型真实产出 3 篇 | 4.0 - 5.0 | 1 |

写「未发现」不写「0」是有意的：评审对真实病句存在漏报，「未发现」只说这一轮没抓到，
不承诺不存在。最后一行样本只有 3 篇，不是合格率 —— 真实文本比夸张样例难改，工具不是零缺陷。
...
早期版本在 [HC3-Chinese](https://github.com/Hello-SimpleAI/chatgpt-comparison-detection)
