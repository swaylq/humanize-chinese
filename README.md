...
> “信息茧房”一词源自 Sunstein 对网络公共领域的讨论，进入短视频研究后，常被直接当作算法推荐后果的同义语使用。这一挪用值得警惕。以抖音、快手为对象的实证研究并未一致支持茧房假设：有研究观察到推荐流中的内容类别随使用时长增加而扩张，用户被推送的题材反而比自主搜索时更杂；Bakshy 等（2015）在社交平台上的发现指向同一方向——用户自身的点击选择对信息窄化的贡献超过排序算法本身。
>
...
> 打完针，抗体不会一直停在高位。多数疫苗接种后一两个月，抗体升到峰值，接下来几个月就明显往下走。这是正常代谢，不是疫苗失效了。
>
> 撑住长期保护的是记忆细胞。它们平时不怎么生产抗体，安静地待在骨髓和淋巴结里，认得那个病原体长什么样。所以血里抗体测不出来，不代表这几针白打了——身体只是把生产线关了，图纸还留着。
>
...
>
> 别拿一次抽血结果决定自己要不要补针。抗体数值和实际保护力之间没有一条干净的换算线，数值偏低的人未必没保护，数值够高也不等于可以永远跳过。按免疫程序走，比看一张化验单可靠。
