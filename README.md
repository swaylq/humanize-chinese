...
题目：写一段关于「短视频信息茧房效应」的学术论文正文。skill 一侧跑了三次，展示的是中位数那一次。
...
**用了 skill，四段跑完** 🟡 40/100 MEDIUM（降 **42 分**，三次分别为 21 / 40 / 46）：

> “信息茧房”被广泛用于描述短视频平台的推荐后果，但这一判断的经验基础比通常设想的要薄。桑斯坦提出该概念时讨论的是用户主动筛选的新闻订阅，而短视频的分发权主要在系统一侧，两种机制不能直接换算。已有实证结果的方向也不统一：一些基于模拟账号的研究观察到，推荐序列的话题范围随交互轮次收窄；另一些追踪真实用户的研究则报告，重度使用者接触的内容类别多于轻度使用者，与茧房假设相反。一个可能的解释是，平台为维持长期留存会主动注入探索性内容，收窄与拓宽在同一系统内同时发生，观测窗口的长度决定了研究者看到哪一面。因此，以单次会话为单位的实验很难支撑关于长期认知窄化的结论。
>
> 现有研究至少受三重限制。外部研究者拿不到分发日志，只能用模拟账号或用户自陈替代，两者都无法把系统推荐与用户的主动回避区分开。“茧房”也缺少统一的操作化指标，话题多样性、来源集中度与观点极化常被混用在同一术语之下。样本则多来自单一平台的短期观测，跨平台与跨文化的可比性尚未建立。在这些条件澄清之前，关于短视频算法造成信息窄化的判断宜作为待检验假设处理。
