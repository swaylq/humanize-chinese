...
实测（1-5 分，3 个模型评审，两轮取并集）：

**仓库里五份样例**（这些是刻意写得很夸张的 AI 腔样本）：
...
**当前模型真实产出的文本**（从 2026-08 新建的语料里抽，更接近你手上的稿子）：
...
三篇里两篇干净，一篇被两位评审指出一处并列失衡
（「孕妇的代谢会明显减慢……吸烟者则会加快」，后半句缺了宾语）。
**样本只有 3 篇，别当成精确的合格率看**，但方向是清楚的：
真实文本比夸张样本难改，工具不是零缺陷。

宁可不改，也不写病句 —— 事实和篇幅那两道关是硬的，通顺关拦不住的会漏一点。

## 关于「AIGC 率」这个分数，先说清楚

这个工具带一个 0-100 的检测分。**它在有些文体上还管用，在另一些上已经不能用了，
甚至是反的。** 下面是 2026-08 用五个当前模型（Claude Opus 5 / GPT-5.6 / DeepSeek V4 /
GLM-5.3 / Kimi K3）实测的结果，每个场景 50-100 篇模型文本对 50 篇真人公开文本，
**两侧长度对齐后**重算：
...
| 学术摘要 | **0.621** | 已经很弱 |
| 小说 | **0.479** | 约等于抛硬币 |
| 长篇博客 | **0.155** | **反的**——真人写的分数更高 |

这个数读法：随机拿一篇模型写的和一篇真人写的，模型那篇分数更高的概率。
**0.5 就是抛硬币。低于 0.5 表示方向反了。**

**所以别拿它筛长文。** 博客那一栏的意思是：你拿它去挑「哪篇像 AI 写的」，
它会更多地挑中真人写的那些。（这一格的样本较少，AI 15 篇对真人 22 篇，
比其它几格弱，但方向明确。）

**还有一件事：真人写的论文也会被判成 AI。**
60 篇真实知网论文摘要（发表时间早于大模型），平均 60 分，
