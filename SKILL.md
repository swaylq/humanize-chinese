...
description: >
  ...
  ④ 纠错收尾 —— 只修错词、病句、标点，其他一个字不动，改动范围有硬校验。
...
| **④ 纠错收尾** | ③ 完成后 | 按下方「流程 ④：纠错清单」只修错词、病句、标点，**其他一个字不动** |

四个流程**顺序执行**。人味在 ① 产生，② 只能修不能补，③ 是机械替换，④ 只纠错。
...
③ 跑完后把稿子再读一遍，**只**修三类：错词/错别字（的地得、搭配错误）、
真病句（成分残缺、前后不接 —— 最小修复，改到通顺就停）、
标点（中文语境半角逗号句号、括号引号不配对、重复标点）。
**风格平淡不是错误；同义词不许换；信息不许增删；句序段落不许动。**
判断口诀：这是一个错误，还是一个可以更好的地方？错误才改。
